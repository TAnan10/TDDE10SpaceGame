...
public class UFOEnemy extends Enemy {
    ...
    // Constructor
    public UFOEnemy(double x, double y, AnchorPane gamePane, int initialHealth, ImageView rocket, LevelOne level) {
    ...
    public void move() {
        ...
        if (movingRight) {
            getX += getSpeed;
            if (getX + enemyImage.getFitWidth() >= LevelOne.Game_Width) {
                movingRight = false;
            }
        } else {
            getX -= getSpeed;
    ...
    private void moveUfoLasers() {
        for (int i = 0; i < ufoLasers.size(); i++) {
            ...
            if (areColliding(ufoLaser, rocket)) {
                ...
                // Deduct a life from the player by calling the method in the LevelOne class
                level.removePlayerLives();
            }

            if (ufoLaser.getLayoutY() > LevelOne.Game_Height) {
                ufoLasers.remove(i);
                gamePane.getChildren().remove(ufoLaser);
                i--;
            }
    ...
    @Override
    public void handleCollision(LevelOne level) {
        checkLaserUFOCollisions(level.getLasers()); // Pass lasers from LevelOne class
    }
    ...
    private void destroy() {
        ...
        // Additional destruction logic or effects if needed
    }
